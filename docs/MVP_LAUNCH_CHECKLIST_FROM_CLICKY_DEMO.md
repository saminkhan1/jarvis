...
| Voice | Hermes supports voice interaction in CLI and messaging platforms, including microphone input, spoken replies, and Discord voice channels. Local STT can work with `faster-whisper` and no API key. | Backend capability exists, but AURA still lacks native macOS microphone UX. Best path is an AURA mic button feeding the normal mission path, not a new agent runtime. connect this to work with our aura like clicky demo |
| MCP | Hermes discovers stdio and HTTP MCP servers, prefixes tools, supports startup registration, resources/prompts where available, and per-server tool filtering. | AURA's CUA daemon proxy is the right boundary. Keep policy filtering in `script/aura-cua-mcp`; avoid native Swift automation first. |
